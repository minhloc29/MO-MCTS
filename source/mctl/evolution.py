import re
import time
from ..common.interface_LLM import InterfaceAPI as InterfaceLLM
import re
...
'" + self.prompt_func_name + "'.\nThis function should accept " + str(
    ...
    def get_prompt_e1(self, indivs: List[Dict[str, Any]]):
        prompt_indiv = ""
...
'" + self.prompt_func_name + "'.\nThis function should accept " + str(
    ...
    def get_prompt_e2(self, indivs: List[Dict[str, Any]]):
        prompt_indiv = ""
        for i in range(len(indivs)):
            # print(indivs[i]['algorithm'] + f"Objective value: {indivs[i]['objective']}")
            prompt_indiv = prompt_indiv + "No." + str(
...
'" + self.prompt_func_name + "'.\nThis function should accept " + str(
    ...
    def get_prompt_m1(self, indiv1: List[Dict[str, Any]]):
        prompt_content = self.prompt_task + "\n" \
...
'" + self.prompt_func_name + "'.\nThis function should accept " + str(
    ...
    def get_prompt_m2(self, indiv1: List[Dict[str, Any]]):
        prompt_content = self.prompt_task + "\n" \
                                            "I have one algorithm with its code as follows. \n\n\
Algorithm's description: " + indiv1['algorithm'] + "\n\
Code:\n\
...
'" + self.prompt_func_name + "'.\nThis function should accept " + str(
    ...
    def get_prompt_s1(self, indivs: List[Dict[str, Any]]):
        prompt_indiv = ""
        for i in range(len(indivs)):
            prompt_indiv = prompt_indiv + "No." + str(
                i + 1) + " algorithm's description, its corresponding code and its objective value are: \n" + \
...
'" + self.prompt_func_name + "'.\nThis function should accept " + str(
    ...
    def e1(self, parents: Dict[str, Any]):
    ...
    def e2(self, parents: Dict[str, Any]):
    ...
    def s1(self, parents: Dict[str, Any]):
